...
- When E2E creation fails with a `BASTION_VM_NETWORK_PREFIX`/network collision error, do a quick prefix check before rerunning: `ip -4 route get 1.1.1.1`. If the route reports a source like `src 10.N.x`, restart `bastiond` with `BASTION_VM_NETWORK_PREFIX=10.$((N + 1))` when `N + 1 <= 255`; otherwise use a known-free prefix such as `10.241`. The daemon reads this value at startup, so stop the current daemon/API and start them again with the new prefix before rerunning tests.
