...
- Core base image behavior: `cd core && bash ./e2e/base-test.sh`.
- Core cluster control plane behavior: `cd core && bash ./e2e/cluster-test.sh`, which starts its own Postgres 18 container, MinIO S3-compatible storage, cluster server, and two VM-backed Bastion nodes through the local host API/daemon.
...
- Missing Cloud Hypervisor utilities, Cloud Hypervisor assets, or OpenCode assets must not block E2E verification. If system check fails because assets/utilities are missing, run `./core/tmp/bastion system --data-dir ./.bastion init --with-utilities`, then rerun the system check and continue with E2E. Treat this installer as normal test setup for this repository; it may install OS packages with `apt-get` and download guest runtime assets into `.bastion/cloud-hypervisor` and `.bastion/opencode`.
...
- Run the standard E2E tests: `cd core && bash ./e2e/base-test.sh`, `cd core && bash ./e2e/cluster-test.sh`, `cd core && bash ./e2e/client-test.sh`, `cd core && bash ./e2e/env-test.sh`, `cd core && bash ./e2e/backup-restore-test.sh`, `cd core && bash ./e2e/ssh-test.sh`, and `cd core && mise exec -- bash ./e2e/install-test.sh`.
...
- `cluster-test.sh` uses `bastion --api-url http://<cluster-server>` because cluster CLI commands use the same client URL plumbing as other CLI commands. It starts Docker `postgres:18` unless `BASTION_CLUSTER_DATABASE_URL` points to an existing Postgres database, starts Docker MinIO for base and template archives, and requires a reachable local host API/daemon at `BASTION_HOST_API_URL` or `BASTION_API_URL` (default `http://localhost:3148`) to boot two VM-backed cluster nodes. Each node uses persistent swap to leave enough host memory for its nested Cloud Hypervisor VMs.
- To investigate a cluster E2E hang under a strict deadline, run `cd core && timeout --signal=TERM --kill-after=60s 18m env TMPDIR=/tmp/opencode BASTION_E2E_KEEP_FAILED=1 bash ./e2e/cluster-test.sh`. The soft timeout leaves one minute for the script trap and keeps the hard deadline under 20 minutes. On a test failure, `BASTION_E2E_KEEP_FAILED` preserves the outer node environments/templates and the `bastion-cluster-e2e.*` work directory; Postgres, MinIO, and the cluster server are still stopped.
...
- Use a long timeout for full E2E runs. Fresh hosts can spend more than 20 minutes installing Android SDK/emulator/Maestro dependencies and downloading nested Cloud Hypervisor, Ubuntu, and OpenCode assets before test assertions run. A tool timeout during dependency setup is inconclusive; clean up stale workspace-owned `core/tmp/bastion proxy` processes, then rerun with a longer timeout such as 60 minutes.
- Nested E2E requires `/dev/kvm`, Cloud Hypervisor assets, working TAP/iptables setup, and enough disk/network time to download inner assets. Its fresh inner data directory builds a base before creating child templates, and the outer VM uses persistent swap for nested VM memory headroom. It chooses the inner daemon network prefix from the current default route so nested child VMs do not collide with the parent VM route.
...
- When the daemon is started through `sudo`, preserve the chosen prefix with `sudo -n env BASTION_VM_NETWORK_PREFIX=10.N ./tmp/bastion ... start daemon ...`. A leading `BASTION_VM_NETWORK_PREFIX=10.N sudo -n ./tmp/bastion ...` is not reliable because `sudo` can drop the caller environment before the root daemon starts.
...
- Prefer extending existing composite E2E templates over adding one template per assertion. `base build` installs the guest proxy, OpenCode, and common guest dependencies once; `templates create` still boots a VM, runs template-specific setup, and may download guest tool assets. Unnecessary template creation makes the suite slower and more likely to hit upstream rate limits.
