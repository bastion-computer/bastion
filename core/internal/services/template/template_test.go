...
func TestServiceAcceptsActionTemplateConfigs(t *testing.T) {
	...
	cases := []struct {
		...
		{key: "opencode-preset-action", config: json.RawMessage(`{"actions":{"init":[{"use":"setup_opencode","with":{"auth":"{\"anthropic\":{\"type\":\"api\",\"key\":\"test-key\"}}","config":"{\"model\":\"anthropic/claude-sonnet-4-5\",\"permission\":\"allow\"}","tui":"{\"mouse\":false,\"keybinds\":{\"input_paste\":\"none\"}}"}}]}}`)},
