...
func NewRouter(db *database.Client) *gin.Engine {
	...
	secrets.GET("/lookup", h.getSecret)
	secrets.DELETE("/lookup", h.removeSecret)
	secrets.POST("/resolve", h.resolveSecret)

	templates := v1.Group("/templates")
	templates.POST("", h.createTemplate)
	templates.GET("", h.listTemplates)
	templates.GET("/lookup", h.getTemplate)
	templates.DELETE("/lookup", h.removeTemplate)
	...
	checkpoints.DELETE("/lookup", h.removeCheckpoint)
