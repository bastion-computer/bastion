...
func (d firecrackerHTTPDownloader) latestRelease(ctx context.Context) (githubRelease, error) {
...
func (d firecrackerHTTPDownloader) downloadReleaseAssets(
	ctx context.Context,
	...
	archiveName := fmt.Sprintf("%s-%s-%s.tgz", firecrackerName, release.TagName, arch)
	...
	manifest.Firecracker = firecrackerName
...
func (d firecrackerHTTPDownloader) downloadGuestAssets(
	ctx context.Context,
	store firecrackerStore,
	arch string,
	tag string,
...
func (d firecrackerHTTPDownloader) releaseChecksum(
	ctx context.Context,
	release githubRelease,
	archiveName string,
	archive githubAsset,
...
func (d firecrackerHTTPDownloader) ciKeys(ctx context.Context, version, arch string) ([]string, error) {
...
func (d firecrackerHTTPDownloader) getJSON(ctx context.Context, target string, out any) error {
...
func (d firecrackerHTTPDownloader) downloadFile(ctx context.Context, source, destination string) error {
...
// extractFirecrackerArchive extracts firecracker and jailer into the store.
//
//nolint:gosec // Archive extraction is constrained to known file basenames in the data directory.
func extractFirecrackerArchive(archivePath, dir, tag, arch string) error {
	...
	for {
		...
		name := firecrackerBinaryName(filepath.Base(header.Name), tag, arch)
	...
	if !found[firecrackerName] || !found[jailerName] {
...
func firecrackerBinaryName(base, tag, arch string) string {
	switch base {
	case fmt.Sprintf("%s-%s-%s", firecrackerName, tag, arch):
