...
func registerResourceRoutes(routes *gin.RouterGroup, handler clusterhandler.Handler) {
	secretRoutes := routes.Group("/secrets")
	...
	templateRoutes := routes.Group("/templates")
	...
	environmentRoutes := routes.Group("/environments")
	environmentRoutes.POST("", handler.CreateEnvironment)
	environmentRoutes.GET("", handler.ListEnvironments)
	environmentRoutes.GET("/by-key/:key", handler.GetEnvironmentByKey)
	environmentRoutes.GET("/by-key/:key/tunnels", handler.EnvironmentTunnels)
	environmentRoutes.DELETE("/by-key/:key", handler.RemoveEnvironmentByKey)
	environmentRoutes.Any("/by-key/:key/agents/:agent", handler.EnvironmentAgentProxy)
	environmentRoutes.Any("/by-key/:key/agents/:agent/*path", handler.EnvironmentAgentProxy)
	environmentRoutes.Any("/by-key/:key/tunnels/:name", handler.EnvironmentTunnelProxy)
	environmentRoutes.Any("/by-key/:key/tunnels/:name/*path", handler.EnvironmentTunnelProxy)
	environmentRoutes.POST("/:id/ssh", handler.EnvironmentSSH)
	environmentRoutes.GET("/:id/tunnels", handler.EnvironmentTunnels)
	environmentRoutes.Any("/:id/agents/:agent", handler.EnvironmentAgentProxy)
	environmentRoutes.Any("/:id/agents/:agent/*path", handler.EnvironmentAgentProxy)
	environmentRoutes.Any("/:id/tunnels/:name", handler.EnvironmentTunnelProxy)
	environmentRoutes.Any("/:id/tunnels/:name/*path", handler.EnvironmentTunnelProxy)
	environmentRoutes.GET("/:id", handler.GetEnvironmentByID)
	environmentRoutes.DELETE("/:id", handler.RemoveEnvironmentByID)
}
